import{_ as e,X as a,Y as o,a1 as n,a0 as i}from"./framework-68dd73a2.js";const t="/blog/assets/Oracle_6_1.png",p="/blog/assets/Oracle_6_2.png",s="/blog/assets/Oracle_6_3.png",r="/blog/assets/Oracle_6_4.png",c="/blog/assets/Oracle_6_5.png",l="/blog/assets/Oracle_6_6.png",g="/blog/assets/Oracle_6_7.png",m="/blog/assets/Oracle_6_8.png",d="/blog/assets/Oracle_6_9.png",f="/blog/assets/Oracle_6_10.png",_="/blog/assets/Oracle_6_11.png",u="/blog/assets/Oracle_6_12.png",b="/blog/assets/Oracle_6_13.png",h="/blog/assets/Oracle_6_14.png",x="/blog/assets/Oracle_6_15.png",y="/blog/assets/Oracle_6_16.png",O="/blog/assets/Oracle_6_17.png",z={},v=i('<h1 id="oracle-简单的查询语句" tabindex="-1"><a class="header-anchor" href="#oracle-简单的查询语句" aria-hidden="true">#</a> Oracle-简单的查询语句</h1><div class="hint-container warning"><p class="hint-container-title">注意</p><p>本博文仅供学术研究和交流参考，严禁将其用于商业用途。如因违规使用产生的任何法律问题，使用者需自行负责。</p></div><ul><li>在启动oracl之前确定这两服务已经打开</li></ul><figure><img src="'+t+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><pre><code>sys/xxx as sysdba
oracle在安装的时候默认把scott账号锁住了，所以需要解锁
conn 用户名/密码
</code></pre><ul><li><p>解锁：登录管理员sys账号</p><pre><code>alter user scott account unlock;
</code></pre><p>如果忘记了scott的密码 修改密码：登录管理员sys账号</p><pre><code> alter user scott identified by 新密码;
</code></pre><p>dept:部门表 deptno部门编号 dname 部门名字 loc部门的地点 emp员工表<br> empno 员工编号 ename 员工名字 job 岗位 mgr 上级经理的员工编号 hiredate 员工入职日期 sal 工资 comm 奖金 deptno 员工所在的部门编号 salgrade 工资等级表； grade 工资等级表； losal 对应等级的最低工资 hisal 对应等级的最高工资</p></li></ul><p>--------------------------第二章 简单的查询语句--------------</p><p>------查询语句 (DQL) 作用：投影数据表的列 ------查询语句的关键字 SELECT -----找出emp表中的全部数据 ----- * 是一个通配符，表示全部了 ----- 想运行某个sql语句，可以选中该语句，点击运行（F8）</p><pre><code>select * from emp;
</code></pre><figure><img src="`+p+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>----关键字；以后可能会自定义一些数据库的对象， ------这些对象的名字不能使用关键字 --子句 关键字后面带着的一个句子叫做子句 --语句 一个可以执行的句子叫做语句</p><p>--sql语句不区分大小写 --语句可以分行，但是分行的时候不能把单词隔开</p><p>--选择全部列</p><pre><code>select * from emp;
...
</code></pre><figure><img src="`+h+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>--1.员工试用期6个月，转正后月薪上调20%， --请查询出所有员工工作第一年的所有收入（需考虑奖金部分)， --要求显示格式为:XXX的第一年总收入为XXX。</p><pre><code>select ename || &#39;第一年总收入为&#39; ||
(sal*6+sal*1.2*6+nvl(comm,0)) as 年薪 
from emp;
</code></pre><figure><img src="`+x+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><p>--2.查询员工表中一共有哪几种岗位类型。</p><pre><code>select job from emp;
</code></pre><figure><img src="`+y+`" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><pre><code>select distinct job from emp;
</code></pre><figure><img src="`+O+'" alt="" tabindex="0" loading="lazy"><figcaption></figcaption></figure><hr><h3 id="分割线" tabindex="-1"><a class="header-anchor" href="#分割线" aria-hidden="true">#</a> 分割线</h3><hr><div class="hint-container info"><p class="hint-container-title">相关信息</p><p><strong>以上就是我关于 <em>Oracle-简单的查询语句</em> 知识点的整理与总结的全部内容，希望对你有帮助。。。。。。。</strong></p></div>',59);function q(E,X){return a(),o("div",null,[n(" more "),v])}const L=e(z,[["render",q],["__file","oracle6.html.vue"]]);export{L as default};
